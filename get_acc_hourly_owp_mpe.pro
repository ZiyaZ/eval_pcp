...
;+
; Accumulate 1-hour OWP Stage IV (MPE) precipitation produced by the
...
;-
      ...
      if (hc eq 0) then begin
          ...
          accMPEGrid = hourlyMPEGrid

      endif else begin

          ind = WHERE((accMPEGrid eq NoDataValue) or $
                      (hourlyMPEGrid eq NoDataValue), count)
          accMPEGrid = accMPEGrid + hourlyMPEGrid
          if (count gt 0) then accMPEGrid[ind] = NoDataValue
