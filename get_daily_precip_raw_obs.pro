PRO GET_DAILY_PRECIP_RAW_OBS, startDate_YYYYMMDD, $
                              finishDate_YYYYMMDD, $
                              hour, $ ; integer
  ...
  windowPadMinutes = 15 ; top of hour plus or minus this many minutes
  ...
  if FILE_TEST(savFilePath) then begin
      ...
      if (startDate_YYYYMMDD__ ne startDate_YYYYMMDD) then STOP
      if (finishDate_YYYYMMDD__ ne finishDate_YYYYMMDD) then STOP
      if (hour__ ne hour) then STOP
...
;
  ...
  HH = STRING(hour, FORMAT = '(I2.2)')
  prevHour = hour - 1 & if (prevHour eq -1) then prevHour = 23
  prevHH = STRING(prevHour, FORMAT = '(I2.2)')
  startDate_Julian = YYYYMMDDHH_TO_JULIAN(startDate_YYYYMMDD + HH)
  finishDate_Julian = YYYYMMDDHH_TO_JULIAN(finishDate_YYYYMMDD + HH)
...
; Get daily precipitation observations.

  queryStart_Julian = startDate_Julian - DOUBLE(windowPadMinutes) / 1440.0D
  queryStart_YYYYMMDDHHMMSS = $
      JULIAN_TO_YYYYMMDDHHMMSS(queryStart_Julian)

  startDate = STRMID(queryStart_YYYYMMDDHHMMSS, 0, 4) + '-' + $
              STRMID(queryStart_YYYYMMDDHHMMSS, 4, 2) + '-' + $
              STRMID(queryStart_YYYYMMDDHHMMSS, 6, 2) + ' ' + $
              STRMID(queryStart_YYYYMMDDHHMMSS, 8, 2) + ':' + $
              STRMID(queryStart_YYYYMMDDHHMMSS, 10, 2) + ':' + $
              STRMID(queryStart_YYYYMMDDHHMMSS, 12, 2)

  queryFinish_Julian = finishDate_Julian + DOUBLE(windowPadMinutes) / 1440.0D
  queryFinish_YYYYMMDDHHMMSS = $
      JULIAN_TO_YYYYMMDDHHMMSS(queryFinish_Julian)
  finishDate = STRMID(queryFinish_YYYYMMDDHHMMSS, 0, 4) + '-' + $
               STRMID(queryFinish_YYYYMMDDHHMMSS, 4, 2) + '-' + $
               STRMID(queryFinish_YYYYMMDDHHMMSS, 6, 2) + ' ' + $
               STRMID(queryFinish_YYYYMMDDHHMMSS, 8, 2) + ':' + $
               STRMID(queryFinish_YYYYMMDDHHMMSS, 10, 2) + ':' + $
               STRMID(queryFinish_YYYYMMDDHHMMSS, 12, 2)

  minusPadStr = STRCRA(60 - windowPadMinutes)
  plusPadStr = STRCRA(windowPadMinutes)
...
;; where diff = min_diff;"
  ...
  if (database eq 'web') then begin
      ...
      statement = 'psql -d web_data -h ' + webPGHost + ' -t -A -c ' + $
                  ...
                  'and t2.date <= ''' + $
                  finishDate + ''' ' + $
                  'and (((date_part(''hour'', date) = ' + prevHH + ') ' + $
                    'and (date_part(''minute'', date) >= ' + $
                     minusPadStr + ')) ' + $
                    'or ((date_part(''hour'', date) = ' + HH + ') ' + $
                    'and (date_part(''minute'', date) <= ' + $
                     plusPadStr + '))) ' + $
...
; Place results in a structure.
  ...
  precipReport = REPLICATE({station_id: '', $
                            ...
                            latitude: 0.0d, $
                            elevation: 0L, $
                            value_meters: REPLICATE(ndv, numDays)}, $
                           numStations)
...
;          valueMeters = FLOAT(report[7])
          value_meters = MAKE_ARRAY(numDays, /FLOAT, VALUE = ndv)
          prevID = id
;print, 'new station ' + id
      ...
      timeInd = ROUND(time_Julian_hour - startDate_Julian)
  ...
  if NOT(FILE_TEST(savFilePath)) then begin
      ...
      ndv__ = ndv
      startDate_YYYYMMDD__ = startDate_YYYYMMDD
      finishDate_YYYYMMDD__ = finishDate_YYYYMMDD
      hour__ = hour
      minLon__ = minLon_
      maxLon__ = maxLon_
      minLat__ = minLat_
      ...
      SAVE, precipReport, numPrecipReports, $
            ndv__, $
            startDate_YYYYMMDD__, finishDate_YYYYMMDD__, hour__, $
